...
var Engine = (function(global) {
    ...
    function renderEntities() {
        /* 遍历在 Game.allEnemies 数组中存放的作于对象然后调用你事先定义的 render 函数 */
        Game.allEnemies.forEach(function(enemy) {
            enemy.render();
        });
     ...
     */
    function reset(dt) {
        if (Game.setGameRun === false) {
            // 游戏失败按钮
            Game.gameOverStatus === true && Game.drawWinOrFailedBtn(dt, 'images/lost.png');

            // 游戏获胜按钮
            Game.winStatus === true && Game.drawWinOrFailedBtn(dt, 'images/win.png');

            // 游戏设置的下拉条
            Game.btnSettingStatus === true && Game.drawSelection();

            // 角色预览
            Game.changeRole === true && Game.previewTheRoles();

            // 最高分
            Game.btnScoreStatus === true && Game.topScore();
        }
    }
